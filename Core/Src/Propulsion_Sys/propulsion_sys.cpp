...
/**
 * @brief Controller for Propulsion System
 * @param tim1 TIM handle
 * @param tim2 TIM handle
 * @retval none
 */
Propulsion_Sys::Propulsion_Sys(TIM_HandleTypeDef *tim1, TIM_HandleTypeDef *tim2)
    : thrust{0},
      motor{// timer2
            T200(tim1, TIM_CHANNEL_1),
            T200(tim1, TIM_CHANNEL_2),
            T200(tim1, TIM_CHANNEL_3),
            T200(tim1, TIM_CHANNEL_4),
            // timer8
            T200(tim2, TIM_CHANNEL_1),
            T200(tim2, TIM_CHANNEL_2),
            T200(tim2, TIM_CHANNEL_3),
            T200(tim2, TIM_CHANNEL_4)}
{
}

void Propulsion_Sys::stop()
{
    for (int i = 0; i < 8; i++)
        motor[i].output(0);
}
